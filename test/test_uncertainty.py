...
class TestOneElement:
    ...
    def test_log(self):  # sourcery skip: bin-op-identity
        ...
        assert unc.log(1, 2, with_value=True) == (np.abs(2 / 1), np.log(1))
        assert unc.log(2, 2, with_value=True)[0] == unc.log(2, 2)
    ...
    def test_log10(self):  # sourcery skip: bin-op-identity
        ...
        assert unc.log10(1, 2, with_value=True) == (np.abs(2 / 1 / np.log(10)),
                                                    np.log10(1))
        assert unc.log10(2, 2, with_value=True)[0] == unc.log10(2, 2)
    ...
    def test_exp(self):
        ...
        assert unc.exp(1, 2,
                       with_value=True) == (np.abs(2 * np.exp(1)), np.exp(1))
        assert unc.exp(2, 2, with_value=True)[0] == unc.exp(2, 2)
    ...
    def test_power(self):  # sourcery skip: bin-op-identity
        ...
        assert unc.power(1, 2, a=1,
                         with_value=True) == (np.abs(1 * 1**(1 - 1) * 2), 1)
        assert unc.power(1, 2, a=2, with_value=True)[0] == unc.power(1, 2, a=2)
    ...
    def test_power_snr(self):
        ...
        for _ in range(10):
            ...
            assert np.isclose(unc.power_snr(x, x_snr, a=a),
                              np.power(x, a) / unc.power(x, x / x_snr, a=a))
    ...
    def test_square(self):
        ...
        assert unc.square(1, 2,
                          with_value=True) == (np.abs(2 * 1**(2 - 1) * 2), 1)
        assert unc.square(1, 2, with_value=True)[0] == unc.square(1, 2)
    ...
    def test_square_snr(self):
        ...
        for _ in range(10):
            ...
            x_snr = np.random.uniform(0.1, 10)
            assert np.isclose(unc.square_snr(x, x_snr),
                              np.square(x) / unc.square(x, x / x_snr))
...
class TestTwoElement:
    ...
    def test_ratio(self):
        ...
        assert unc.ratio(1, 2, 0.1, 0.2,
                         with_value=True) == (np.abs(1 / 2) *
                                              np.sqrt((0.1 / 1)**2 +
                                                      (0.2 / 2)**2), 1 / 2)
        assert unc.ratio(6, 3, 1, 1,
                         with_value=True)[0] == unc.ratio(6, 3, 1, 1)
    ...
    def test_multiply(self):
        ...
        assert unc.multiply(1, 1, 2, 2,
                            with_value=True) == (np.sqrt((1 * 2)**2 +
                                                         (1 * 2)**2), 1)
        assert unc.multiply(1, 2, 3, 4,
                            with_value=True)[0] == unc.multiply(1, 2, 3, 4)


class TestNElement:

    def test_sum(self):  # sourcery skip: bin-op-identity
        assert unc.sum([1, 2, 3], [1, 2, 3]) == np.sqrt(1**2 + 2**2 + 3**2)

        assert unc.sum([1, 2, 3], [1, 2, 3],
                       w=[1, 1,
                          1]) == np.sqrt((1 / 3)**2 + (2 / 3)**2 + (3 / 3)**2)
        assert unc.sum([1, 2, 3], [1, 2, 3],
                       w=[1, 1, 0]) == np.sqrt((1 / 2)**2 + (2 / 2)**2)
        assert unc.sum([1, 2, 3], [1, 2, 3],
                       w=[2, 2,
                          2]) == np.sqrt((1 / 3)**2 + (2 / 3)**2 + (3 / 3)**2)

        assert unc.sum([1, 2, 3], [1, 2, 3],
                       a=[1, 1, 1]) == np.sqrt(1**2 + 2**2 + 3**2)
        assert unc.sum([1, 2, 3], [1, 2, 3], a=[1, 1,
                                                0]) == np.sqrt(1**2 + 2**2)
        assert unc.sum([1, 2, 3], [1, 2, 3],
                       a=[2, 2,
        ...
        assert unc.sum([1, 2, 3], [1, 2, 3],
                       with_value=True) == (np.sqrt(1**2 + 2**2 + 3**2), 6)
        assert unc.sum([1, 2, 3], [1, 2, 3],
                       with_value=True)[0] == unc.sum([1, 2, 3], [1, 2, 3])
